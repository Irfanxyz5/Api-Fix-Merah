...
export default async function handler(req, res) {
  ...
  if (duration === 'permanent') {
    ...
    if (!keyRegex.test(customApiKey)) return res.status(400).json({ error: 'Custom API Key hanya boleh huruf, angka, underscore (_), strip (-), panjang 8-64 karakter' });
  ...
  // --- Panggil API Qiospay untuk buat QRIS ---
  ...
  if (!qiosData.qr_image_url) {
    // Hapus transaksi jika gagal buat QRIS
